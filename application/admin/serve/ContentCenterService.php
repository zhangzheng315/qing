...
class ContentCenterService extends Common{
     ...
     */
    public function  contentCenterInfo($param)
    {
        $id = $param['id'];
        $where = ['id' => $id];
        $info = $this->content_center->find($where);
     ...
     * @return bool
     */
    public function contentCenterEdit($param)
    {
        $where = ['id' => $param['id']];
        $data = [
            'order' => $param['order'] ?: 0,
        ...
        $rs = $this->content_center->update($data,$where);
        if(!$rs){
            $this->setError('修改失败');
            Db::rollback();
            return false;
